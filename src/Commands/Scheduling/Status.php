...
class Status extends Command
{
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $table
            ...
            ->setRows([
                [
                    'scheduler' , (!is_null(getSchedulingProcessId()) && Process::kill(getSchedulingProcessId(), 0)) ? '<fg=#C3E88D;options=bold> ACTIVE </>' : '<fg=#FF5572;options=bold> DEACTIVE </>' , getSchedulingProcessId()
                ]
            ]);
        $table->setVertical();
        $table->render();
