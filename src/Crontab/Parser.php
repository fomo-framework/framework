...
class Parser
{
    public function parse(string $crontabString, $startTime = null): array
    {
        if (! $this->isValid($crontabString)) {
            (new Log())->channel('scheduling')->warning('Invalid cron string: ' . $crontabString);
    ...
    private function parseStartTime($startTime): int
    {
        ...
        if (! is_numeric($startTime)) {
            (new Log())->channel('scheduling')->warning("\$startTime have to be a valid unix timestamp ($startTime given)");
